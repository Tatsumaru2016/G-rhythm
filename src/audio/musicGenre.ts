...
function detectOnsets(buffer: AudioBuffer, minGap: number): Onset[] {
  ...
  for (let i = 0; i < data.length; i += windowSize) {
    ...
    if (flux > 0.008 && energy > 0.015 && time - lastOnset >= minGap) {
...
function estimateBpmFromOnsets(onsets: Onset[]): number {
